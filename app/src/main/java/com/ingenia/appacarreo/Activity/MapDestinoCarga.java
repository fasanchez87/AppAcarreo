...
public class MapDestinoCarga extends AppCompatActivity implements OnMapReadyCallback,
        ActivityCompat.OnRequestPermissionsResultCallback, OnPermissionCallback
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.activity_map_destino_carga);

        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);

        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbarDestino);
        setSupportActionBar(toolbar);
        SupportMapFragment mapFragment = (SupportMapFragment) getSupportFragmentManager()
                .findFragmentById(R.id.mapDestino);
        mapFragment.getMapAsync(this);

        gestionSharedPreferences = new gestionSharedPreferences(getApplicationContext());


        mTimerIsRunning = false;


        autoCompleteTextDireccionDestino=(AutoCompleteTextView) findViewById(R.id.autoCompleteEditTextDireccionDestino);

        progressBarEsperaDireccionDestino = (ProgressBar) findViewById(R.id.progressBarEsperaDireccionDestino);
        buttonCargaDestino = (Button) findViewById(R.id.buttonCargaDestino);
        buttonCargaDestino.setOnClickListener(new View.OnClickListener()
        {
            @Override
            public void onClick(View view)
            {
                direccionTextoClienteDestino = TextUtils.isEmpty(autoCompleteTextDireccionDestino.getText().toString())?"":
                        autoCompleteTextDireccionDestino.getText().toString();

                //Intent intent = new Intent(MapDestinoCarga.this, Servicio.class);
                gestionSharedPreferences.putString("direccionTextoClienteDestino",direccionTextoClienteDestino);
                gestionSharedPreferences.putString("latitudDestino",latitudDestino);
                gestionSharedPreferences.putString("longitudDestino",longitudDestino);
                Log.i("peligro", " ::: " +gestionSharedPreferences.getString("direccionTextoClienteDestino"));
                Log.i("peligro", " ::: " +gestionSharedPreferences.getString("latitudDestino"));
                Log.i("peligro", " ::: " +gestionSharedPreferences.getString("longitudDestino"));


               /* intent.putExtra("direccionTextoClienteDestino", direccionTextoClienteDestino);
                intent.putExtra("latitudDestino", latitudDestino);
                intent.putExtra("longitudDestino", longitudDestino);
                Log.i("Destino", " : " +direccionTextoClienteDestino );
                Log.i("Destino", " : " +latitudDestino );
                Log.i("Destino", " : " +longitudDestino );
*/
    ...
    private void enableMyLocation()
    {
        ...
        if (mGoogleMap != null)
        {
            progressBarEsperaDireccionDestino.setVisibility(View.VISIBLE);
            ...
            mGoogleMap.setOnCameraMoveStartedListener(new GoogleMap.OnCameraMoveStartedListener()
            {
                ...
                mTimerIsRunning = true;*/
                    mTimerIsRunning = true;
                    isMoving = true;
                    progressBarEsperaDireccionDestino.setVisibility(View.GONE);
            ...
            mGoogleMap.setOnCameraIdleListener(new GoogleMap.OnCameraIdleListener()
            {
                ...
                public void onCameraIdle()
                {
                    ...
                    latitudDestino = ""+mGoogleMap.getCameraPosition().target.latitude;
                    longitudDestino = ""+mGoogleMap.getCameraPosition().target.longitude;
                    runOnUiThread(new Runnable()
                    {
                        @Override
                        public void run()
                        {
                            autoCompleteTextDireccionDestino.setText(""+getCompleteAddressString(mGoogleMap.getCameraPosition().target.latitude,
                                    mGoogleMap.getCameraPosition().target.longitude));
                            autoCompleteTextDireccionDestino.setSelection(autoCompleteTextDireccionDestino.getText().length());
    ...
    public android.support.v7.app.AlertDialog getAlertDialog(final String permission)
    {
        ...
        builder.setButton(DialogInterface.BUTTON_POSITIVE, "Entiendo", new DialogInterface.OnClickListener()
        {
            ...
            public void onClick(DialogInterface dialog, int which)
            {
                ...
                openSettingsScreen(MapDestinoCarga.this);
            }
        });
